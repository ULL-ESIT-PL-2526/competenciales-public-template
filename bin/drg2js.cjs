...
const generateCode = require('../src/codegen.cjs');
const fs = require('fs');
...
program
    ...
    .description('Transforms Dragon source code into JS or LLVM IR code')
    .option('-o, --output <fileName>', 'Output file name with generated JavaScript code (default: <input>.js)')
    .option('-a, --ast', 'Output the Babel AST as JSON')
    .option('-g --codegen <babel|manual|llvm>', 'Code generation: "babel" to use @babel/generator, "manual" to traverse the AST and generate code, "llvm" to generate LLVM IR', 'manual')
    ...
    .option('--skip-scope-analysis', 'Skip scope and type analysis phases', false)
    .addHelpText('after', `
If option --codegen llvm is specified, the options --sandbox and --pretty will be ignored. 
...
If option --ast and option --verbose are specified together, the AST structure will be printed to stderr in a human-readable format using insp() with depth: null.
...
async function main() {
    ...
    let extension = (options.codegen === 'llvm') ? '.ll' : '.js';
    /*
    if (!options.output) {
        options.output = inputFile.toLowerCase().endsWith('.drg')
            ? `${inputFile.slice(0, -4)}${extension}`
            : `${inputFile}${extension}`;
    }
    */
    if (options.codegen === 'llvm') {
        if (options.sandbox) {
            console.warn('Warning: --sandbox option is ignored when --codegen llvm is used');
        }
        if (options.pretty) {
            console.warn('Warning: --pretty option is ignored when --codegen llvm is used');
        }
    ...
    try {
        ...
        const { code, map } = generateCode(ast, options, input, inputFile);
